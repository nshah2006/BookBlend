...
export function MoodInput() {
  ...
  const moods = [
    ...
    { id: "melancholic", icon: <CloudRain className="w-8 h-8" />, label: "Melancholic", color: "text-blue-400 bg-blue-400/10" },
    { id: "curious", icon: <Brain className="w-8 h-8" />, label: "Curious", color: "text-purple-400 bg-purple-400/10" },
    { id: "romantic", icon: <Heart className="w-8 h-8" />, label: "Romantic", color: "text-pink-400 bg-pink-400/10" },
    { id: "intense", icon: <Flame className="w-8 h-8" />, label: "Intense", color: "text-red-400 bg-red-400/10" },
  ...
  return (
    <div className="container mx-auto px-6 py-12 max-w-4xl min-h-[80vh] flex flex-col justify-center">
      <div className="text-center mb-16 space-y-4">
        ...
        <h1 className="text-4xl md:text-6xl font-serif text-foreground">How are you <span className="text-primary italic">feeling</span> today?</h1>
        <p className="text-foreground/60 text-lg">BookBlend's AI will find the perfect companion for your current headspace.</p>
      </div>
      ...
      <div className="bg-card rounded-[3rem] p-8 md:p-12 shadow-2xl border-2 border-primary/5 space-y-16 relative overflow-hidden">
        ...
        <div className="flex justify-center gap-4 mb-8">
          {[1, 2, 3].map(s => (
            <div 
              key={s} 
              className={`w-3 h-3 rounded-full transition-all duration-500 ${step >= s ? "bg-primary scale-125 w-10" : "bg-foreground/10"}`} 
            />
          ))}
        </div>
        ...
        {step === 1 && (
          ...
          >
            <h3 className="text-2xl font-serif text-center text-foreground">Pick your current emotional state:</h3>
              ...
              {moods.map(m => (
                <button
                  ...
                  className={`p-8 rounded-[2rem] border-2 transition-all flex flex-col items-center gap-4 group hover:shadow-xl hover:-translate-y-2 ${
                    mood === m.id ? "border-primary bg-primary/5" : "border-white/5 bg-background/50"
                  }`}
                >
                  <div className={`p-4 rounded-2xl transition-all ${mood === m.id ? "bg-primary text-primary-foreground" : `${m.color} group-hover:scale-110`}`}>
                    {m.icon}
                  </div>
            ...
            <div className="space-y-12">
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-primary font-bold">{pacing < 40 ? "Slow & Thoughtful" : pacing > 70 ? "Fast & Frenetic" : "Steady & Balanced"}</span>
                </div>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-secondary font-bold">{depth < 40 ? "Light & Playful" : depth > 70 ? "Devastatingly Deep" : "Emotionally Resonant"}</span>
                </div>
                <div className="relative h-12 flex items-center">
                  <input 
                    type="range" 
        ...
        {step === 3 && (
          ...
          >
            ...
            <p className="text-foreground/60 max-w-md mx-auto">
              Our AI is ready to scour the multiverse for books that match your <span className="text-primary font-bold uppercase">{mood}</span> mood, with <span className="text-secondary font-bold">{pacing}%</span> intensity and <span className="text-primary font-bold">{depth}%</span> emotional depth.
            </p>
