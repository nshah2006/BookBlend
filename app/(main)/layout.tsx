    ...
    return (
      <div className="fixed inset-0 bg-background flex flex-col items-center justify-center z-[200]">
        ...
        >
          ...
          <p className="text-foreground/60 mt-2 font-sans tracking-widest uppercase text-xs">
            Stirring the cauldron of stories...
          </p>
            ...
            <div className="flex flex-col gap-6">
              {navItems.map((item) => (
                <Link
                  ...
                  className={`flex items-center gap-4 text-xl font-serif p-4 rounded-2xl transition-all ${
                    pathname === item.path
                ...
                >
                  <span
                    className={
                      pathname === item.path ? "text-secondary" : "text-tertiary"
                    }
                  >
                    {item.icon}
                  </span>
              ...
              {isAuthenticated ? (
                <button
                  onClick={() => void handleLogout()}
                  className="w-full bg-primary text-primary-foreground py-4 rounded-xl font-bold text-center"
                >
                  Sign Out
                </button>
              ) : (
          ...
          >
            <div
              className={`p-2 rounded-xl ${
                pathname === item.path ? "bg-tertiary/10" : ""
              }`}
            >
              {item.icon}
            </div>
            <span className="text-[10px] font-bold uppercase tracking-widest">
              {item.name === "Home" ? "Main" : item.name}
            </span>
          </Link>
        ))}
      </nav>

      <footer className="bg-primary text-primary-foreground py-16 px-6 relative z-10 hidden md:block mb-0 lg:mb-0">
        <div className="container mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
          <div className="space-y-4">
            ...
            <p className="text-primary-foreground/70 max-w-xs">
              AI-powered reading companion that matches your heart&apos;s current mood
              with the perfect story.
            </p>
          </div>
          <div>
            <h4 className="font-bold mb-6 text-tertiary uppercase tracking-widest text-xs">
              Moods
            </h4>
            <ul className="space-y-4 text-primary-foreground/80">
              <li>
                <Link href="/mood-input" className="hover:text-secondary">
                  Emotional Depth
                </Link>
              </li>
              <li>
          ...
          <div>
            ...
            <ul className="space-y-4 text-primary-foreground/80">
              ...
              <li>
                <Link href="/challenges" className="hover:text-secondary">
                  Reading Quests
                </Link>
              </li>
              <li>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                type="email"
                placeholder="Email..."
                className="bg-primary-foreground/10 border border-primary-foreground/20 rounded-lg px-4 py-2 flex-grow text-sm focus:outline-none focus:border-tertiary"
              />
              <button className="bg-tertiary text-tertiary-foreground px-4 py-2 rounded-lg font-bold hover:bg-tertiary/90">
                Join
              </button>
            </div>
          </div>
