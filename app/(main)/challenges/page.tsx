...
export default function Challenges() {
  ...
  const [leaderboard, setLeaderboard] = useState<
    Array<{ rank: number; name: string; xp: number; level: number; isCurrentUser?: boolean }>
  >([]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        toast.error(
          error instanceof Error ? error.message : "Unable to load quest data."
        );
      }
    };
      ...
      <div className="container mx-auto px-6 py-12 max-w-5xl pb-24 lg:pb-12">
        <div className="mt-24 p-12 bg-primary rounded-[3rem] text-center space-y-8 relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <h2 className="text-4xl font-serif text-primary-foreground italic">
              Quest tracking starts after sign-in
            </h2>
            <p className="text-primary-foreground/70 max-w-2xl mx-auto text-lg font-light leading-relaxed">
              Challenges, XP, levels, and the seasonal leaderboard all come from your
              authenticated backend profile.
            </p>
            <Link
              href="/auth"
              className="inline-flex bg-secondary text-secondary-foreground px-10 py-5 rounded-2xl font-bold text-lg hover:bg-secondary/90 transition-all hover:scale-105 shadow-xl border-2 border-secondary group"
            >
              Enter the Sanctuary{" "}
    ...
    <div className="container mx-auto px-6 py-12 max-w-5xl pb-24 lg:pb-12">
      <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-16">
        <div className="space-y-4">
          ...
          <p className="text-foreground/60 text-lg">
            Your backend quest progress updates from recommendations, reading, and
            completed books.
          </p>
        </div>
        <div className="flex bg-card rounded-2xl p-4 shadow-xl border-2 border-primary/5 items-center gap-6 self-start md:self-auto">
          <div className="text-center">
            <span className="block text-2xl font-serif font-bold text-tertiary">
              {summary.totalXp}
            </span>
      ...
      <div className="mb-12 rounded-2xl border border-primary/10 bg-card p-6 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
        <p className="text-foreground/70">
          <span className="font-bold text-foreground">{summary.completedChallenges}</span>{" "}
          completed quests and{" "}
          <span className="font-bold text-foreground">{summary.activeChallenges}</span>{" "}
          active.
        </p>
        <p className="text-foreground/70">
          Challenge completion rate:{" "}
          <span className="font-bold text-tertiary">{summary.completionRate}%</span>
        </p>
        <p className="text-foreground/70 md:text-right">
          Next level target:{" "}
          <span className="font-bold text-secondary">{summary.nextLevelXp} XP</span>
        </p>
      </div>
          ...
          >
            ...
            <div className="space-y-6">
              <div className="p-4 bg-background rounded-2xl w-fit shadow-lg shadow-black/5 group-hover:scale-110 transition-transform">
                {icons[challenge.id as keyof typeof icons] ?? (
                  <Trophy className="w-8 h-8 text-secondary" />
                )}
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl font-serif font-bold text-foreground group-hover:text-tertiary transition-colors">
                  {challenge.title}
                </h3>
                <p className="text-foreground/60 font-medium">{challenge.desc}</p>
              </div>
            </div>
            ...
            <div className="space-y-4 pt-8 border-t border-primary/5">
              <div className="flex justify-between items-end text-xs font-bold uppercase tracking-widest">
                <span className="text-foreground/40">Quest Progress</span>
                ...
                <motion.div
                  ...
                  className={`h-full bg-gradient-to-r ${
                    challenge.isCompleted
                      ...
                      : "from-tertiary/50 to-tertiary"
                  } rounded-full shadow-[0_0_10px_rgba(212,175,55,0.3)]`}
                />
              </div>
              <div className="flex justify-between items-center pt-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-foreground/30 flex items-center gap-1">
                ...
                <button
                  className={`p-2 rounded-lg transition-all ${
                    challenge.isCompleted
                      ...
                      : "hover:bg-primary/5 text-foreground/20"
                  }`}
                >
