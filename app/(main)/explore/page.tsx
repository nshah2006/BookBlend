...
import { addBookToLibrary, getBooks, getLatestRecommendation } from "@/lib/api";
...
export default function Explore() {
  ...
  const recommendationIds = useMemo(
    () =>
      new Set(
        getLatestRecommendation()?.recommendations.map((entry) => entry.book.id) ?? []
      ),
    []
  );
  ...
  useEffect(() => {
    const loadGenres = async () => {
      try {
        ...
        toast.error(
          error instanceof Error ? error.message : "Unable to load book filters."
        );
      }
    };
  ...
  return (
    <div className="container mx-auto px-6 py-12 pb-24 lg:pb-12">
      {/* Header & Search */}
      <div className="space-y-12 mb-16">
        <div className="text-center max-w-2xl mx-auto space-y-4">
          ...
          <h1 className="text-4xl md:text-6xl font-serif text-primary italic">
            The Great Glade Library
          </h1>
          <p className="text-foreground/60 text-lg">
            Every book here has been touched by the magic of the forest. Sift through the
            leaves to find your next soul-warming tale.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 max-w-4xl mx-auto relative group">
          ...
          <div className="relative flex-grow flex items-center bg-card border-2 border-primary/10 rounded-2xl px-6 py-4 focus-within:border-secondary transition-all shadow-xl">
            <Search className="text-secondary w-6 h-6 mr-4" />
            <input
              type="text"
              placeholder="Search by title, author, or spell..."
              className="bg-transparent border-none outline-none w-full text-lg text-foreground placeholder:text-foreground/30"
              value={searchQuery}
          ...
          <div className="relative flex items-center bg-card border-2 border-primary/10 rounded-2xl px-6 py-4 focus-within:border-secondary transition-all shadow-xl min-w-[200px]">
            <Filter className="text-secondary w-5 h-5 mr-4" />
            <select
              className="bg-transparent border-none outline-none w-full text-foreground font-medium appearance-none"
              value={selectedGenre || ""}
          ...
          {genres.map((genre) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-bold transition-all border shadow-sm ${
                selectedGenre === genre
                  ? "bg-secondary text-secondary-foreground border-secondary shadow-secondary/20 scale-105"
                  : "bg-card text-foreground/60 border-primary/10 hover:border-secondary hover:text-secondary"
              }`}
            >
              {genre}
            </button>
      ...
      {recommendationIds.size > 0 && (
        <div className="mb-8 max-w-4xl mx-auto rounded-2xl border border-secondary/20 bg-secondary/10 px-6 py-4 text-center text-foreground">
          Your latest vibe blend is active. Recommended titles are pinned to the top of
          this catalog.
        </div>
      )}
      ...
      {books.length > 0 ? (
              ...
              >
                <div className="relative aspect-[3/4.5] rounded-[2rem] overflow-hidden mb-6 shadow-xl group-hover:shadow-secondary/30 transition-all border-2 border-transparent group-hover:border-secondary/40">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-primary/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-8">
                    <p className="text-white text-sm line-clamp-3 mb-4 italic">
                ...
                <div className="space-y-1">
                  <h3 className="text-xl font-serif text-foreground group-hover:text-secondary transition-colors truncate">
                    {book.title}
                  </h3>
                  ...
                  <div className="flex items-center gap-1 pt-2">
                    ...
                    <span className="text-[10px] font-bold text-secondary ml-1">
                      {book.rating}
                    </span>
                  </div>
                </div>
        ...
        <div className="text-center py-24 space-y-6">
          ...
          <h3 className="text-3xl font-serif text-foreground/60">
            No tales found in this glade...
          </h3>
          <p className="text-foreground/40">
            Try whispering a different keyword to the forest spirits.
          </p>
          <button
            onClick={() => {
              setSearchQuery("");
        ...
        {selectedBook && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 sm:p-12">
            ...
            >
              ...
              <div className="w-full md:w-3/5 p-8 md:p-16 overflow-y-auto space-y-10">
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-2 mb-4">
                    {selectedBook.genre.map((g) => (
                      <span
                        key={g}
                        className="text-[10px] font-bold uppercase tracking-widest text-secondary border border-secondary/30 px-3 py-1.5 rounded-full bg-secondary/5"
                      >
                        {g}
                      </span>
                    ))}
                  ...
                  <div className="flex items-center gap-6 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-foreground/40 text-sm">/ 5.0</span>
                    </div>
                ...
                <div className="space-y-4">
                  <h4 className="uppercase tracking-[0.2em] text-xs font-bold text-foreground/40">
                    The Lore
                  </h4>
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-10 border-t border-primary/5">
                  ...
                  <button
                    ...
                    className="bg-card border-2 border-primary/10 text-foreground px-10 py-5 rounded-2xl font-bold text-lg hover:border-secondary hover:text-secondary transition-all flex items-center justify-center gap-3"
                  >
