...
import { deleteLibraryItem, getLibrary, updateLibraryItem } from "@/lib/api";
...
export default function MyLibrary() {
  ...
  const removeItem = async (bookId: string) => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Unable to remove that book."
      );
    }
  };
  ...
  if (!isAuthenticated) {
    return (
      <div className="container mx-auto px-6 py-12 pb-24 lg:pb-12">
        <div className="text-center py-32 space-y-8 bg-card/30 backdrop-blur-md rounded-[3rem] border-2 border-dashed border-primary/10">
          ...
          <div className="space-y-2">
            <h3 className="text-4xl font-serif text-foreground/60 italic">
              Sign in to unlock your archive
            </h3>
            <p className="text-foreground/40 text-lg max-w-md mx-auto">
              Your saved books, reading progress, and completed titles live here once you
              authenticate.
            </p>
          </div>
          <Link
            href="/auth"
            className="inline-flex bg-secondary text-secondary-foreground px-10 py-5 rounded-2xl font-bold text-lg hover:bg-secondary/90 transition-all hover:scale-105 shadow-xl border-2 border-secondary"
          >
            Enter the Sanctuary
          </Link>
        </div>
    ...
    <div className="container mx-auto px-6 py-12 pb-24 lg:pb-12">
      <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-16 border-b border-primary/10 pb-12">
        <div className="space-y-4">
          ...
          <h1 className="text-4xl md:text-6xl font-serif text-foreground italic">
            The Keeper&apos;s Archive
          </h1>
          <p className="text-foreground/60 text-lg">
            Live data from your library, including shelves and progress.
          </p>
        </div>

        <div className="flex bg-card/50 backdrop-blur-md p-1.5 rounded-2xl border border-primary/10 shadow-xl self-start md:self-auto">
          {tabs.map((tab) => (
            <button
              key={tab.id}
      ...
      {isLoaded && filteredItems.length > 0 ? (
        <div className="space-y-12">
          {filteredItems.map((item, index) => (
            <motion.div
              ...
              className="group bg-card rounded-[2.5rem] border-2 border-primary/5 p-8 flex flex-col md:flex-row gap-12 hover:shadow-2xl hover:border-secondary/30 transition-all relative overflow-hidden"
            >
              ...
              <div className="flex-grow flex flex-col justify-between py-4 space-y-8">
                <div className="space-y-4">
                  ...
                  <p className="text-foreground/60 text-lg line-clamp-2 italic">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {item.book.genre.map((genre) => (
                      <span
                        key={genre}
                        className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 border border-primary/10 px-3 py-1.5 rounded-full bg-primary/5 group-hover:border-secondary/20 transition-colors"
                      >
                        {genre}
                      </span>
                    ))}
                ...
                <div className="flex flex-col sm:flex-row items-center gap-12 border-t border-primary/5 pt-8">
                  ...
                  <div className="flex gap-4 w-full sm:w-auto">
                    ...
                    <button
                      ...
                      className="p-4 bg-card border-2 border-primary/10 text-destructive/40 hover:text-destructive hover:border-destructive/20 rounded-2xl transition-all shadow-sm"
                    >
        ...
        <div className="text-center py-32 space-y-8 bg-card/30 backdrop-blur-md rounded-[3rem] border-2 border-dashed border-primary/10">
          ...
          <div className="space-y-2">
            ...
            <p className="text-foreground/40 text-lg max-w-md mx-auto">
              Head to Explore to save books into this shelf and see the backend update in
              real time.
            </p>
          </div>
          <Link
